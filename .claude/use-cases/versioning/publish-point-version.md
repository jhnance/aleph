...
- `POST /api/orgs/:orgSlug/points/:id/versions` accepts `versionSemantic` (semver string), `versionClassification` (`'release' | 'prerelease' | 'hotfix' | 'metadata'`), `exports` (array of `{ name, predecessorExportId? }`), optional `connections` (array of `{ toVersionId, type? }`), and for `frontend_component` points an optional `props` array of `{ name, propType?, required?, defaultValue?, description? }`; requires an authenticated session with an active org
