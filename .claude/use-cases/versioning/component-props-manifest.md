...
- Props are submitted as part of the `POST /api/orgs/:orgSlug/points/:id/versions` payload for `frontend_component` versions (see Publish Point Version); each prop entry has `name`, and optional `propType`, `required` (boolean, default false), `defaultValue`, and `description`
...
- `GET /api/orgs/:orgSlug/points/:id/versions/:versionId/props` returns the full props manifest for that version: each prop's stable `id`, `name`, `propType`, `required`, `defaultValue`, `description`
