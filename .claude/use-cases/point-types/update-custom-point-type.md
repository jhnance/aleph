...
- `PATCH /api/custom-point-types/:id` accepts `name`; requires an authenticated session with an active org
...
- If the update returns 0 rows — whether because the type does not exist or belongs to another org — the server returns 403; no distinction is made between the two cases to avoid leaking tenant information
