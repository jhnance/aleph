...
- On confirmation, the CLI uploads demo artifacts to S3 via pre-signed URLs **before** the version commit, then calls `POST /api/orgs/:orgSlug/points/:id/versions` with the assembled payload: one `useCases` entry per discovered `.aleph.ts` — `{ lineageId: file id, title: file title, exportName: file export field, demoArtifactUrl: uploaded URL }` (see Publish Point Version) — `point_version_use_cases` rows are written once, complete, with a demo always present (ordering and payload decided 2026-06-10)
- The version's use case set is exactly what the payload declares — there is no server-side forward propagation (2026-06-10). A use case present on the previous version but absent here does not appear on the new version; the lock-file scan preflight is what makes that omission deliberate
- On success, the CLI prints: published version string, export count, use case count (with demo artifact count). The internal `version_monotonic` tiebreaker is never surfaced to users
