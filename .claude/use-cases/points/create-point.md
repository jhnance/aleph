...
- `POST /api/orgs/:orgSlug/points` accepts `name`, `domainId`, `type` (`frontend_component` | `custom`), and type-specific metadata; requires an authenticated session with an active org
