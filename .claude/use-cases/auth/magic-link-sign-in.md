...
- Accepted risk — **login CSRF** (2026-06-11): `GET /api/auth/verify` is a state-changing GET, but it derives no authority from the session cookie (the emailed token is the credential), so classic CSRF gains nothing. An attacker *can* navigate a victim's browser through the attacker's own magic link, signing the victim into the attacker's account; the payoff against a catalog tool is negligible and mitigating it would fight the magic-link UX. Documented, not mitigated.
