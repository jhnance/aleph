...
- `POST /api/orgs/:orgSlug/use-cases/:useCaseId/edit` creates a new `draft_use_cases` row pre-populated with the existing use case's `title` and `content`, and `lineage_id` set to the use case's `lineage_id`
